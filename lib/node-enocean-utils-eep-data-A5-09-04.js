...
EnoceanUtilsEepDataParser.prototype.parse = function(dd_buf) {
	...
	var db2 = dd_buf.readUInt8(1);
	var db1 = dd_buf.readUInt8(2);
	var db0 = dd_buf.readUInt8(3);
	// Humidity
	var hum = db3;
	var hum_result = hum * 5 / 10;
	var hum_desc = hum_result + ' %';
	// Concentration
	var conc = db2;
	var conc_result = conc * 10;
	var conc_desc = conc_result + ' ppm';
	// Temperature
	var tmp = db1;
	var tmp_result = tmp * 2 / 10;
	var tmp_desc = tmp_result + ' ℃';
