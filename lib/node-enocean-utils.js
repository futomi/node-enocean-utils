...
EnoceanUtils.prototype._parseDataUnknown = function (buf, offset, len) {
	let buffer = buf.slice(offset, offset + len);
	let hex = mBuffer.convBufferToHexString(buffer);
	let parsed = {
		'field': 'Data',
		'message': {},
...
EnoceanUtils.prototype._parseDataErp2 = function (buf, offset, len) {
	...
	if (eha === 0b1) {
		offset++;
		let eheader_value = buf.readUInt8(offset);
		let eheader_buffer = buf.slice(offset, offset + 1);
		let eheader_hex = mBuffer.convBufferToHexString(eheader_buffer);
		let eheader_bin = ('0000000' + eheader_value.toString(2)).slice(-8);

		let rc = (eheader_value && 0b11110000) >> 4;
		let rc_map = {
			0: 'Original telegram',
			15: 'Original telegram, do not repeat this telegram'
		};
		let rc_desc = rc_map[rc] || 'Telegram level repeated: ' + rc;

		let lo = eheader_value && 0b00001111;
		let lo_map = {
			0: 'No optional data field in frame'
		};
		let lo_desc = lo_map[lo] || 'Length of optional data field: ' + lo + ' Bytes';
		...
		let eheader_values = {
			'RC': {
				...
				'value': rc,
				'hex': [mBuffer.convDecToHexString(rc)],
				'bin': eheader_bin.substr(0, 4),
				'desc': rc_desc
			},
			'LO': {
				'key': 'LO',
				'field': 'Length of Optional data',
				'value': lo,
				'hex': [mBuffer.convDecToHexString(lo)],
				'bin': eheader_bin.substr(4, 4),
				'desc': lo_desc
			}
		};
		let ext_header = {
			...
			'bin': eheader_bin,
			'hex': eheader_hex,
			'buffer': eheader_buffer,
			'values': eheader_values,
			'structure': [eheader_values['RC'], eheader_values['LO']]
		};
		structure.push(ext_header);
	}

	// Extended Telegram type
	if (tt === 0b1111) {
		offset++;
		let ett_value = buf.readUInt8(offset);
		let ett_buffer = buf.slice(offset, offset + 1);
		let ett_hex = mBuffer.convBufferToHexString(ett_buffer);
		let ett_bin = ('0000000' + ett_value.toString(2)).slice(-8);

		let ett_map = {
			0x00: 'SYS_EX telegram (0xC5)',
			0x01: 'Smart Ack Learn request telegram (0xC6)',
			0x02: 'Smart Ack Learn Answer (0xC7)',
			0x03: 'Chained data message (0x40)',
			0x04: 'secure telegram (0x32)',
			0x05: 'Generic Profiles Teach-in request (0xB0)',
			0x06: 'Generic Profiles Teach-in response (0xB1)',
			0x07: 'Generic Profiles Complete data (0xB2)'
		};
		let ett_desc = ett_map[ett_value] || 'Original R-ORG: 0x' + ett_hex;
		...
		let ett = {
			'field': 'Extended Telegram type',
			'value': ett_value,
			'bin': ett_bin,
			'hex': ett_hex,
			'buffer': ett_buffer,
			'values': ett_values,
			'structure': [ett_values['TT']]
		};
		structure.push(ett);
	}
